...
interface Props extends HTMLAttributes<HTMLDivElement> {
  calendarRange?: CalendarRange; // 달력 범위 (없을 시 기본값 적용)
...
const DatePicker: React.FC<Props> = ({
  ...
  return (
    <div className={`shadow-neumorphism p-30 ${props.className ?? ''}`}>
      <DatePickerHeader calendarRange={calendarRangeInit} showingDate={showingDate} setShowingDate={setShowingDate} />
      <div className='flex items-center'>
        {days.map((day) => (
          <div key={day} className='flex justify-center items-center grow py-10 text-[#728197] text-20 font-medium'>
            {day}
          </div>
        ))}
      </div>
